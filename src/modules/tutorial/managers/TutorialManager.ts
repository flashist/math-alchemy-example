...
export class TutorialManager extends BaseAppManager {
    ...
    public completeStep(id: string): void {
        if (!this.checkIfStepActive(id)) {
            return;
        }

        const stepController: TutorialStepController = this.findControllerByStepId(id);
        stepController.complete();
    }

    protected findControllerByStepId(id: string): TutorialStepController {
        let result: TutorialStepController;

        for (let singleController of this.activeStepControllers) {
            if (singleController.stepId === id) {
                result = singleController;
                break;
            }
        }
