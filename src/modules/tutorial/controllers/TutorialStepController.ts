...
export class TutorialStepController extends BaseObjectWithGlobalDispatcher {
    ...
    protected baseStepInfoToViewPadding: number = 20;
    ...
    protected onStageDown(): void {
        if (this.stepConfig.completeOnStageDown) {
    ...
    protected createContent(): void {
        const tutorialCont: FContainer = this.containersManager.getContainer(TutorialContainerId);
        ...
        this.stepViewCont.interactiveChildren = this.stepViewCont.interactive;
    ...
    // protected holeWasCreated: boolean;
    ...
    protected updateTutorialView(): void {
        ...
        this.targetView = this.containersManager.getContainer(this.stepConfig.viewId);
        if (this.targetView) {
            ...
            // Holes
            const holePadding: number = this.stepConfig.targetViewHolePadding || 0;
            this.stepBg.beginHole();
            this.stepBg.drawRect(
                this.targetViewBounds.x - holePadding,
                this.targetViewBounds.y - holePadding,
                this.targetViewBounds.width + holePadding * 2,
                this.targetViewBounds.height + holePadding * 2
            );
            this.stepBg.endHole();

            // this.holeWasCreated = true;
        }
        ...
        let contY: number = 0;
        if (this.targetView) {
            contY = this.targetViewBounds.y + Math.floor((this.prevTargetViewBounds.height - this.stepScaledCont.height) / 2);
            if (this.stepConfig.valign) {
                if (this.stepConfig.valign === VAlign.TOP) {
                    contY = this.targetViewBounds.y - this.stepScaledCont.height - this.baseStepInfoToViewPadding;
                } else if (this.stepConfig.valign === VAlign.BOTTOM) {
                    contY = this.targetViewBounds.y + this.targetViewBounds.height + this.baseStepInfoToViewPadding;
                }
            }

        } else {
            contY = Math.floor((screenHeight - this.stepScaledCont.height) / 2);
            if (this.stepConfig.valign) {
                if (this.stepConfig.valign === VAlign.TOP) {
                    contY = Math.floor(screenHeight - this.stepScaledCont.height - this.baseStepInfoToViewPadding);
                } else if (this.stepConfig.valign === VAlign.BOTTOM) {
                    contY = Math.floor(screenHeight + this.targetViewBounds.height + this.baseStepInfoToViewPadding);
                }
            }
        }

        this.stepScaledCont.x = contX;
        // if (this.stepConfig.infoToViewPaddingX) {
        //     this.stepScaledCont.x += this.stepConfig.infoToViewPaddingX;
        // }
        this.stepScaledCont.y = contY;
        // if (this.stepConfig.infoToViewPaddingY) {
        //     this.stepScaledCont.y += this.stepConfig.infoToViewPaddingY;
        // }
    }
}