...
import { Align, FLabel, FLabelTextType, getText, Graphics, Sprite } from "@flashist/flibs";
...
export class PreloaderPageView extends BasePageView {
    ...
    set viewProgress(value: number) {
        ...
        if (this.viewProgress >= 1) {
            this.emit(PreloaderPageViewEvent.PROGRESS_COMPLETE as any);
        }
    }
    ...
    protected arrange(): void {
        ...
        this.titleLabel.x = this.logo.x + Math.floor((this.logo.width - this.titleLabel.width) / 2);;
        this.titleLabel.y = this.logo.y - this.titleLabel.height;
